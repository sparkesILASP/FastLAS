...
void Solver::ILASP::operator()(
    const std::function<void(const std::string &)> &final_fn) const {
  ...
  if (pid < 0) {
    ...
    auto ret = system(std::string("ILASP-release " + args + " " + inpipe +
                                  " > " + outpipe + " 2> /dev/null")
                          .c_str());
    exit(0);
  } else {
...
#else
  auto ret = system(
      string("ILASP " + args + " " + inpipe + " > " + outpipe + " 2> /dev/null")
          .c_str());
#endif
